...
private:
    ...
    public:
        ...
        node* next = nullptr;
        node* prev = nullptr;
        node(const value_type& val, node* next, node* prev)
            : val(val)
            , next(next)
            , prev(prev)
        {
        }
        ~node() {if (next!=nullptr) delete next; if (prev!=nullptr) delete prev;}
    };
...
public:
    ...
    public:
        Iter(LinkedList* list, int index)
            : list(list)
            , index(index){};
 ...
 * @return void
 */
template <typename value_type> void LinkedList<value_type>::push_back(const value_type& val)
{
    if(head == nullptr)
    {
        assert(tail==nullptr);
 ...
 */
template <typename value_type> const value_type& LinkedList<value_type>::at(int index) const
{
    // index is zero based if nodes = 1 it's index = 0;
    //      Therfor last_index = nodes -1;
    if(index >= nodes)
        throw out_of_range("Attempted to access invalid index in list");
 ...
 */
template <typename value_type> value_type& LinkedList<value_type>::at(int index)
{
    // index is zero based if nodes = 1 it's index = 0;
    //      Therfor last_index = nodes -1;
    if(index >= nodes)
